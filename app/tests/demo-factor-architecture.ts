...
async function demonstrateFactorArchitecture() {
  ...
  console.log('🎭 Architecture: Actor = A class of MCP client that provides DSL tooling interfaces');
  ...
  try {
    ...
    console.log(`   📊 Total Market Cap: $${(simulatedMarketData.analytics.totalMarketCap / 1e12).toFixed(2)}T`);
    ...
    if (publishResult.success) {
      ...
      console.log(`   🔗 Streaming responses: ${publishResult.responses.length} successful publishes`);
    ...
    console.log(`   🎭 Factor 1 (CoinGecko): Connected=${factor1Status.isConnected}, Queries=${factor1Status.totalQueries}`);
    console.log(`   📡 Factor 2 (Publisher): Connected=${factor2Status.isConnected}, Published=${factor2Status.publishCount}`);

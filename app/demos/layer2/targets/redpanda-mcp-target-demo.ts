...
import type { CryptoPriceData, CryptoOHLCVData, CryptoMarketAnalytics, Level1Data } from "../../../../lib/src/dsl";
  ...
  coins.forEach(coin => {
    exchanges.forEach(exchange => {
      const basePrice = coin.coinId === "bitcoin" ? 45000 : 
                       coin.coinId === "ethereum" ? 3000 :
                       coin.coinId === "solana" ? 100 : 0.5;
...
function generateLevel1Data(): Level1Data[] {
  ...
  tickers.forEach(ticker => {
    exchanges.forEach(exchange => {
      const midPrice = ticker === "BTC-USD" ? 45000 : 
                      ticker === "ETH-USD" ? 3000 : 100;
...
async function demonstrateMCPStreamingPublish() {
  ...
  try {
    ...
    if (isSuccess(singlePriceResult)) {
      ...
      console.log(`   ${priceData[0].name}: $${priceData[0].usdPrice.toLocaleString()} → crypto-prices topic [${priceData[0].exchangeId}]`);
    } else {
    ...
    console.log(`✅ ${successfulOHLCV}/${Math.min(10, ohlcvData.length)} OHLCV entries published to stream`);
    
    if (successfulOHLCV > 0) {
      console.log("   Sample OHLCV messages published:");
      ohlcvData.slice(0, 3).forEach((candle, index) => {
        console.log(`     ${index + 1}. ${candle.coinId} [${candle.exchangeId}] ${candle.timeframe}: $${candle.openPrice.toFixed(2)} → $${candle.closePrice.toFixed(2)}`);
      });
    }
    ...
    const level1Results = await Promise.all(
      level1Data.map(l1 => writer.publishLevel1(l1))
    );
    ...
    if (successfulLevel1 > 0) {
      ...
      level1Data.slice(0, 3).forEach((l1, index) => {
        console.log(`     ${index + 1}. ${l1.ticker} [${l1.exchange}]: Bid=$${l1.bestBid.toFixed(2)} Ask=$${l1.bestAsk.toFixed(2)} Spread=${l1.spreadPercent}%`);
      });
    }
  ...
  try {
    ...
    if (isSuccess(initResult)) {
      ...
      const batchResults = await Promise.all(
        batches.map(batch => writer.publishPrices(batch))
      );
      ...
      console.log(`   Throughput: ${Math.round(largePriceDataset.length / (duration / 1000))} messages/second`);
      console.log(`   MCP server optimized streaming pipeline`);
      
      // Show streaming statistics
      const exchangeStats = largePriceDataset.reduce((acc, item) => {
    ...
    if (isSuccess(initResult)) {
      ...
      for (let i = 0; i < maxIterations; i++) {
        ...
        if (isSuccess(publishResult)) {
          console.log(`  📈 Update ${i + 1}/${maxIterations}: Published at ${timestamp.toISOString()}`);
          realTimeData.forEach(price => {

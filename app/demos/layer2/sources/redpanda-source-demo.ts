...
async function demonstrateRedpandaSource() {
  ...
  try {
    ...
    const pricesResult = await redpandaReader.getCurrentPrices(["bitcoin", "ethereum", "cardano"], {
      vsCurrencies: ["usd"],
    ...
    if (isSuccess(level1Result)) {
      ...
      if (level1Data) {
        ...
        console.log(
          `       📊 Spread: $${level1Data.spread.toFixed(2)} (${level1Data.spreadPercent.toFixed(2)}%)`,
        );
      } else {

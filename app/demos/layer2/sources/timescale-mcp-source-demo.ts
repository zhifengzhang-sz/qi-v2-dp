...
async function demonstrateHistoricalPriceQuery() {
  ...
  try {
    ...
    if (isSuccess(historyResult)) {
      ...
      console.log(`✅ Retrieved ${prices?.length || 0} historical price points`);

      if (prices && prices.length > 0) {
        console.log("\n📈 Recent Bitcoin Prices:");
        prices.slice(-5).forEach((price, index) => {
          console.log(
            `  ${index + 1}. $${price.usdPrice.toLocaleString()} (${price.lastUpdated.toISOString()}) [${price.exchangeId}]`,
          );
        });

        // Calculate price statistics
        const currentPrice =
          prices && prices.length > 0 ? prices[prices.length - 1]?.usdPrice || 0 : 0;
        const oldestPrice = prices && prices.length > 0 ? prices[0]?.usdPrice || 0 : 0;
      ...
      console.error(
        ...
        getError(historyResult)?.message || "Unknown error",
      );
    }
    ...
    if (isSuccess(ohlcvResult)) {
      ...
      console.log(`✅ Retrieved ${ohlcvData?.length || 0} OHLCV data points`);

      if (ohlcvData && ohlcvData.length > 0) {
        console.log("\n📊 Ethereum OHLCV Data:");
        (ohlcvData || []).forEach((candle, index) => {
          console.log(
            `  Day ${index + 1}: Open=$${candle.open.toFixed(2)} High=$${candle.high.toFixed(2)} Low=$${candle.low.toFixed(2)} Close=$${candle.close.toFixed(2)} [${candle.exchangeId}]`,
          );
        });
      }
    } else {
    ...
    if (isSuccess(tickersResult)) {
      ...
      console.log(`✅ Retrieved ${tickers?.length || 0} available tickers`);

      if (tickers && tickers.length > 0) {
        console.log("\n🏷️  Available Cryptocurrencies:");
        (tickers || []).forEach((ticker, index) => {
          console.log(
            `  ${index + 1}. ${ticker.name} (${ticker.symbol.toUpperCase()}) - ${ticker.coinId} [${ticker.exchangeId}]`,
          );
        });
      }
    } else {
      console.error(
        "❌ Failed to get available tickers:",
        getError(tickersResult)?.message || "Unknown error",
      );
    }

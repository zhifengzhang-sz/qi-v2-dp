...
import { createDataAcquiringAgent, type CryptoDataRequest } from '../publishers/data.acquiring.agent';
import { createDataStoreAgent, type StreamConsumerConfig } from '../consumers/data.store.agent';
...
export class CryptoDataPlatformOrchestrator {
  ...
  async demonstrateManualFlow(): Promise<void> {
    ...
    try {
      ...
      console.log(`✅ Published ${publishMetrics.successfulPublishes} records to topics:`, publishMetrics.topics);
  ...
  async monitorPlatform(): Promise<{
    ...
    const platformStatus = {
      ...
      dataFlow: publisherStatus.isRunning && consumerStatus.isConsuming ? 
        'flowing' : 'interrupted'
    };
    ...
    console.log(`🎯 Success Rate: ${(consumerStatus.successfulInserts / consumerStatus.totalRecordsProcessed * 100).toFixed(1)}%`);
  ...
  async demonstrateUseCases(): Promise<void> {
    ...
    console.log('\n💼 Use Case 3: Portfolio monitoring');
    const portfolioSymbols = ['bitcoin', 'ethereum', 'cardano', 'solana', 'polkadot', 'chainlink', 'uniswap'];
    await this.dataAcquiringAgent.acquireSpecificData(
      portfolioSymbols,
      ['price', 'market_data']
    );

...
export class MCPClient {
  ...
  private async handlePostgresToolCall(tool: string, params: any): Promise<any> {
  ...
  private async handleCoinGeckoToolCall(tool: string, params: any): Promise<any> {
  ...
  private async handleKafkaToolCall(tool: string, params: any): Promise<any> {
...
import { type MCPToolRegistry, createMCPToolRegistry } from "../mcp-tools/registry";
import { CryptoDataPublisher } from "../publishers/crypto-data-publisher";
import type { CryptoOHLCV, CryptoPrice, MarketAnalytics } from "../publishers/types";
...
export class CryptoPlatformAgent extends BaseAgent {
  ...
  async setupDatabaseTables(): Promise<void> {
  ...
  async collectCryptoData(
    symbols: string[] = ["bitcoin", "ethereum", "cardano", "solana"],
  ): Promise<void> {
    ...
    try {
      ...
      for (const [coinId, data] of Object.entries(priceData as Record<string, any>)) {
        const priceMessage: CryptoPrice = {
          coin_id: coinId,
          symbol: coinId.toUpperCase(),
          timestamp: Date.now(),
          usd_price: (data as any).usd,
          btc_price: (data as any).btc,
          market_cap: (data as any).usd_market_cap,
          volume_24h: (data as any).usd_24h_vol,
          change_24h: (data as any).usd_24h_change,
          last_updated: (data as any).last_updated_at,
        };
  ...
  async collectOHLCVData(symbol: string, days = "1"): Promise<void> {
    ...
    try {
      ...
      for (const [timestamp, open, high, low, close, volume] of ohlcvData) {
        const ohlcvMessage: CryptoOHLCV = {
          coin_id: symbol,
  ...
  async analyzeMarketData(): Promise<MarketAnalytics> {
    ...
    try {
      ...
      const analytics: MarketAnalytics = {
        ...
        defi_market_cap: topMarkets
          .filter((coin: any) => coin.categories?.includes("decentralized-finance-defi"))
          .reduce((sum: number, coin: any) => sum + coin.market_cap, 0),
  ...
  async monitorPlatform(): Promise<void> {
  ...
  async orchestratePlatform(): Promise<void> {

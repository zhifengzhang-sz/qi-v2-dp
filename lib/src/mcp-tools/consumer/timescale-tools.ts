...
import { TimescaleClient, type TimescaleQueryOptions } from "../../base/database/timescale-client";
import type { MCPTool } from "../registry";

/**
 * High-Performance Time-Series Query Tool
 */
export class TimeSeriesQueryTool implements MCPTool {
  ...
  async execute(params: {
    ...
    try {
      ...
      const options: TimescaleQueryOptions = {
      ...
      switch (params.query) {
        ...
        case "ohlcv_range":
          if (!options.timeRange) throw new Error("Time range required for OHLCV query");
        ...
        case "custom":
          if (!params.customSQL) throw new Error("Custom SQL required");
      ...
      throw new Error(
        `TimescaleDB query failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
}
 ...
 */
export class TimeSeriesInsertTool implements MCPTool {
  ...
  async execute(params: {
    ...
    try {
      ...
      throw new Error(
        `TimescaleDB insertion failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  private async insertBatch(table: string, data: any[]): Promise<void> {
    ...
    switch (table) {
      case "crypto_prices":
        await this.client.insertPrices(
          data.map((item) => ({
 ...
 */
export class HypertableManagementTool implements MCPTool {
  ...
  async execute(params: {
    ...
    try {
      ...
      switch (params.operation) {
        ...
        case "retention":
          throw new Error(
            `Operation '${params.operation}' is handled automatically during initialization`,
          );
      ...
      throw new Error(
        `Hypertable management failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
}

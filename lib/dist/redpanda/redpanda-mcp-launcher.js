...
export class OfficialRedpandaMCPLauncher {
    ...
    async waitForReady() {
        ...
        try {
            ...
            throw new Error(`Redpanda MCP Server startup failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    getStatus() {
        return {
    ...
    getAvailableTools() {
        ...
        if (this.config.useCloudMCP) {
            tools.push("list_cloud_clusters", "describe_cloud_cluster", "create_cloud_topic", "manage_cloud_acls");
        }
        return tools;
    }

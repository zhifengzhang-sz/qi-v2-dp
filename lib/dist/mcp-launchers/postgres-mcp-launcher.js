...
export class OfficialPostgresMCPLauncher {
    ...
    async start() {
        ...
        this.process = spawn("npx", ["-y", "@modelcontextprotocol/server-postgres", this.config.connectionString], {
            env: {
                ...process.env,
    ...
    async waitForReady() {
        ...
        try {
            ...
            throw new Error(`PostgreSQL MCP Server startup failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    getStatus() {
        return {
    ...
    getAvailableTools() {
        ...
        if (!this.config.readOnly) {
            ...
            "execute_query", "execute_insert", "execute_update", "execute_delete", "create_table", "alter_table", "drop_table");
        }
        return tools;
    }

    ...
    async execute(params) {
        ...
        try {
            switch (params.operation) {
                ...
                case "publish_price":
                    if (!params.priceData)
                        throw new Error("Price data required");
                    await this.producer.publishPrice(params.priceData);
                    break;
                case "publish_ohlcv":
                    if (!params.ohlcvData)
                        throw new Error("OHLCV data required");
            ...
            throw new Error(`Streaming operation failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
}
/**
    ...
    async execute(params) {
        ...
        try {
            switch (params.operation) {
                ...
                case "subscribe":
                    if (!params.topics)
                        throw new Error("Topics required for subscribe");
            ...
            throw new Error(`Consumer operation failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
}
/**

...
export async function setupTimescaleDB(): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    ...
    try {
      ...
      await testClient.query(
        `SELECT create_hypertable('market_analytics', 'timestamp', if_not_exists => TRUE);`,
      );
    ...
    await testClient.query(
      "CREATE INDEX IF NOT EXISTS idx_crypto_ohlcv_coin_timestamp ON crypto_ohlcv(coin_id, timestamp);",
    );
    await testClient.query(
      "CREATE INDEX IF NOT EXISTS idx_market_analytics_timestamp ON market_analytics(timestamp);",
    );

...
import { isSuccess, isFailure, getData, getError } from '../../../src/qicore/base';
import type { MarketDataReadingDSL, CryptoPriceData, CryptoOHLCVData, CryptoMarketAnalytics, Level1Data } from '../../../src/abstract/dsl';
  ...
  describe('Result<T> Pattern Compliance', () => {
    it('should return Result<T> from all methods', async () => {
      const methods = [
        ...
        () => reader.getOHLCVByDateRange({ ticker: 'bitcoin', dateStart: new Date(), dateEnd: new Date(), interval: '1d' }),

...
/**
 * CoinGecko Market Data Reader - External Integration Tests
 *
...
describe("CoinGeckoMarketDataReader", () => {
  ...
  let expectedBitcoinPrice: number;

  beforeAll(async () => {
    // Validate fixtures are available before testing
    const validation = await testDataLoader.validateFixtures();
    if (!validation.valid) {
      throw new Error(`Test fixtures invalid: ${validation.errors.join(", ")}`);
    }

    // Load expected values from real data
    expectedBitcoinPrice = await testDataLoader.getCurrentBitcoinPrice();
    console.log(`📊 Using real Bitcoin price from fixtures: $${expectedBitcoinPrice}`);
  ...
  describe("DSL Interface Implementation", () => {
    ...
    it("should get global market analytics", async () => {
      ...
      if (isSuccess(result)) {
        ...
        // Validate against reasonable ranges based on fixtures (allow 50% variance for market volatility)
  ...
  describe("Handler Architecture Verification", () => {
    beforeEach(async () => {
      if (!reader.getStatus().isInitialized) {
        await reader.initialize();
      }
    ...
    it("should implement only handler methods, not DSL methods", () => {
      // This verifies the handler architecture - concrete class implements handlers,
      // DSL methods are inherited from BaseReader
      const prototype = Object.getPrototypeOf(reader);
      const methods = Object.getOwnPropertyNames(prototype);

      // Should have handler method implementations
  ...
  describe("Error Handling", () => {
    ...
    it("should handle error cases properly", async () => {
      // Create an uninitialized reader to force an error
      const uninitializedReader = createCoinGeckoMarketDataReader({
        name: "error-test-reader",
        debug: false,
      });

      const result = await uninitializedReader.getCurrentPrice("bitcoin");

...
describe('QiCore 2-Layer Architecture', () => {
  ...
  it('should have CoinGecko actor implementation', async () => {
    const { CoinGeckoMarketDataReader, createCoinGeckoMarketDataReader } = await import('../src/sources/coingecko');
  ...
  it('should verify plugin architecture pattern', async () => {
    ...
    // Verify DSL methods are inherited (not reimplemented)
    const readerPrototype = Object.getPrototypeOf(reader);
    const basePrototype = Object.getPrototypeOf(readerPrototype);
    
    // DSL methods should be on BaseReader prototype, not CoinGecko prototype
    expect(typeof reader.getCurrentPrice).toBe('function');
  ...
  it('should have proper Result type integration', async () => {
    const { success, failure, isSuccess, isFailure, getData, getError, createQiError } = await import('../src/qicore/base');

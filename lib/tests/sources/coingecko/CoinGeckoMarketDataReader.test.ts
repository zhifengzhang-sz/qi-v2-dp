...
import { CoinGeckoMarketDataReader, createCoinGeckoMarketDataReader } from '../../sources/coingecko';
import type { CryptoPriceData, CryptoOHLCVData, CryptoMarketAnalytics } from '../../abstract/dsl';
  ...
  describe('DSL Interface Implementation', () => {
    ...
    it('should get OHLCV data', async () => {
      ...
      if (isSuccess(result)) {
        ...
        console.log(`✅ OHLCV - O: $${ohlcv.open}, H: $${ohlcv.high}, L: $${ohlcv.low}, C: $${ohlcv.close}`);
      } else {
        const error = getError(result);
